...
export const OverviewCard = ({user, audienceType, audience, network, isUp, today
  ...
  return (
    <article className="bg-Light-Grayish-Blue w-full h-[216px] mb-4 rounded-[5px]
     mx-auto overflow-hidden  text-center dark:bg-Dark-Desaturated-Blue hover:brightness-95 cursor-pointer hover:dark:brightness-125">
       <div className={`${networkColors[network]} h-[4px] mb-8`}></div>
        ...
        <div className='flex items-center place-content-center gap-1'>
            ...
            <p className={`text-xs font-bold ${isUp ? 'text-Lime-Green' 
                : 'text-Bright-Red'}`}> {today} Today</p>
        </div>
    </article>
  )
}
